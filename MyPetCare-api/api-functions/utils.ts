...
export async function createAppointmenInDatabase(date:Date, hour:string, type:string, reason:string, petId:string, vetId:string) {

    const hourSplit = hour.split(":");
    const hoursFromJournal = [parseInt(hourSplit[0]), parseInt(hourSplit[1])];

    if (hoursFromJournal[0] > 17 || hoursFromJournal[0] < 9 || (hoursFromJournal[0] == 17 && hoursFromJournal[1] > 0)) {
...
export async function updateBlockedDate(clinicId:string, databaseIndex:string, day:string, buttonId:number, appointmentId:string):Promise<functionResponse> {
