  ...
  const response = await fetch(FirestorePrescriptionURL, {
    ...
    body: JSON.stringify({
      fields: {
        ...
        text: { stringValue: prescription.text },
        ...
        milligrams: { integerValue: prescription.milligrams },
      }
    }),
...
export async function updatePrescription(ctx: RouterContext<"/api/putPrescription/:id">) {
  ...
  const allowedFields = ["name", "text", "id_pet", "id_vet", "milligrams"];
  const fields: Record<string, any> = {};
  const updateMask: string[] = [];

  for (const field of allowedFields) {
    if (receta[field] !== undefined && receta[field] !== null) {
      if (typeof receta[field] === "number") {
        fields[field] = { integerValue: receta[field] };
      } else {
