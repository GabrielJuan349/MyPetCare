...
export async function newAppointment(ctx: RouterContext<"/api/appointment/:id">) {
    ...
    const appointmentId = await createAppointmenInDatabase(dateFormated, time, type, reason, petId, vetId);
    if (!appointmentId) {
        ctx.response.status = 500;
    ...
    const {done, error} = await updateBlockedDate(clinicId, databaseIndex, day, buttonId, appointmentId);
